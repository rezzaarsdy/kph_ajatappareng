...
<div class="content-wrapper">
    ...
    <div class="row">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
    <form action="{{ route('profile.update', $profile->uuid) }}" method="POST" id="quickForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    ...
                    <select id="profile_category_id" name="profile_category_id" class="form-control select_role" onchange="tampilkan()">
                        ...
                        @foreach ($kategori as $us)
                            <option value="{{ $us->id }}" @if ($profile->profile_category_id == $us->id) selected @endif>{{$us->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea type="name" min="0" id="content" name="content" value="{{ old('content') }}" class="form-control tinymce-editor">{{$profile->content}}</textarea>
                </div>
            </div>
            <!-- /.card-body -->
        </div>
