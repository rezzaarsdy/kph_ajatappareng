...
<div class="content-wrapper">
    ...
    <div class="row">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
    <form action="{{ route('perhutanan.update', $perhutanan->id) }}" method="POST" id="quickForm" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select id="perhutanan_category_id" name="perhutanan_category_id" class="form-control select_role" onchange="tampilkan()">
                        ...
                        @foreach ($kategori_perhutanan as $us)
                            <option value="{{ $us->id }}" @if ($perhutanan->perhutanan_category_id == $us->id) selected @endif>{{$us->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="name" min="0" id="title" name="title" value="{{ $perhutanan->title }}" class="form-control" placeholder="Masukkan Kepanjangan" >
                </div>

                <div class="form-group" id="form1">
                    <label>Link</label>
                    @error('link') <span style="font-size: 12px; color:red; display: block;">{{ $message }}</span> @enderror
                    <input type="name" min="0" id="link" name="link" value="{{ $perhutanan->link }}" class="form-control" placeholder="Masukkan Link Map contoh : http://www.google.com">
                </div>

                <div class="form-group">
                    <label>Content</label>
                    @error('content') <span style="font-size: 12px; color:red; display: block;">{{ $message }}</span> @enderror
                    <textarea class="form-control tinymce-editor" style="height:50px" name="content">{{$perhutanan->content}}</textarea>
                </div>
            </div>
            <!-- /.card-body -->
        </div>
